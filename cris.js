...
document.addEventListener('DOMContentLoaded', () => {
  ...
  input.addEventListener('keydown', e => { if (e.key === 'Enter') authenticate(); });
  ...
  lumeLink.addEventListener('click', e => {
    e.preventDefault();
    lumeLink.textContent = 'FONTE EM REVISÃO // SPOILERS EM MANUTENÇÃO';
    setTimeout(() => { lumeLink.innerHTML = 'ACESSAR SITE DA LUME <span>↗</span>'; }, 2600);
  });
  ...
  if (samuelPlayer && samuelAudio) {
    ...
    // Atualiza o tempo assim que o navegador conhece a duração do MP3
    // e continua atualizando durante a reprodução.
